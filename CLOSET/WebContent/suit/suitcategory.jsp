    ...
    <!--product-->       
        <div class="product">
            ...
            <%
                ...
                totalCount = cdao.KindCount(Kind);
            ...
            /* String name = (String)session.getAttribute("sessionName"); */
             ...
             ArrayList<closetBean> sKind= cdao.getRegisterKind(startRow, endRow, Kind);
             
    		 
             //3개씩 보여주기
             int j=0;
             for(int i=0; i< sKind.size();i++){
            	 closetBean cbean=sKind.get(i);
            ...
            <div class="bd-color-grey">
               <div class="page_number">   
                  ...
                  %>
                      <a href="suitcategory.jsp?Kind=<%=Kind%>&&pageNum=<%=endPage-pageBlock %>" class="item"> 이전 </a>
                  <% 
                      } 
                      ...
                      for(int s=startPage; s<=endPage; s++){
                         if(s == pageNum){
                  %>
                            <a href="suitcategory.jsp?Kind=<%=Kind%>&&pageNum=<%=s %>" class="item"><b> <%=s %> </b></a>
                  <%
                         }
                         else{
                  %>
                            <a href="suitcategory.jsp?Kind=<%=Kind%>&&pageNum=<%=s %>" class="item"> <%=s %> </a>
                  <% 
                         }
                      }
                      ...
                      if(endPage<pageCount){
                  %>
                  	  <a href="suitcategory.jsp?Kind=<%=Kind%>&&pageNum=<%=startPage+pageBlock %>" class="item"> 다음 </a>
                  <% 
                       }
                   }
